// Mutation 5 — mut2 + Y[0] direct expression.
// Y[0] = m_a AND m_b AND ~lb_a AND ~lb_b under our encoding (proven by Cirbo at 4 gates).
// We can hard-route Y[0] from raw bits, bypassing the mag[0]→y[0] path.
...
module fp4_mul (
    ...
    // Hard-routed Y[0]: P[0] (mag[0] = P[0] when K=0; mag[0]=0 otherwise)
    // mag[0] = P[0] AND (K==0). K==0 ⟺ ~a[2] AND ~b[2].
    // P[0] = ma AND mb. So y[0] = ma & mb & ~a[2] & ~b[2].
    assign y[0] = ma & mb & ~a[2] & ~b[2];

    // mut2-style "below" detection
    wire below1 = ~mag[0];
