// Mutation 14 — combine "below" detection with running-OR P_nonzero shortcuts.
// Observation: below_i for i ≥ first-shifted-bit always becomes ~ANY_i where
// ANY_i grows as the shift accumulates. We can replace some below_i's with
// running-OR or skip them when mag[j]=0 trivially.
// Specifically, mag[0] = P[0] & isK0, and isK0 = ~eh_a & ~eh_b under σ. So:
//   mag[0] = a[0]&b[0]&~a[2]&~b[2] (same as Y[0])
// Use a hardcoded mag[0] and let the rest follow.
...
module fp4_mul (
    ...
    // Hardcode mag[0] = ma & mb & ~a[2] & ~b[2] (= Y[0] when sy=0; passthrough always)
    wire mag0_h = ma & mb & ~a[2] & ~b[2];
