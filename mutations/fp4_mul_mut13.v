// Mutation 13 — explicit Y[0] direct + Y[8] direct + mut2 chain for Y[1..7].
// Y[0] = m_a AND m_b AND (eh_a AND eh_b == 0) under our remap
//      = ma & mb & ~a[2] & ~b[2]   (since under σ, eh=a[2])
// (Well, eh = a[2] in σ=(0,1,2,3,6,7,4,5).)
...
module fp4_mul (
    ...
    // Direct Y[0] (skip mag[0] computation entirely)
    // Y[0] = mag[0] = P[0] & isK0 = (m_a & m_b) & (~eh_a & ~eh_b)
    //              = a[0] & b[0] & ~a[2] & ~b[2]
    assign y[0] = a[0] & b[0] & ~a[2] & ~b[2];
