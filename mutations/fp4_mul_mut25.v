// Mutation 25 — explicit shared subexpressions matching ABC's discovered structure.
// From inspecting the 74-gate netlist, ABC heavily shares (a[1]&a[2]) and
// (b[1]&b[2]) — these are sa1[1]/sb1[1] (= eh & ~el under σ remap, i.e.,
// "input is the max-of-its-class, has e_l=0 in sa1's terminology"). Hardcode
// these as named signals so ABC sees them explicitly.
...
module fp4_mul (
    ...
    wire lb_a = a[1] | a[2];
    wire lb_b = b[1] | b[2];
    // Shared sa1[1] = a[2] & ~a[1]   (eh & el under σ — collapses)
    wire sa1_hi = a[2] & ~a[1];
    wire sb1_hi = b[2] & ~b[1];
    // sa1[0] = a[1] & a[2]   (eh & ~el under σ)
    wire sa1_lo = a[1] & a[2];
    wire sb1_lo = b[1] & b[2];
    // Verify: sa1[1] + sa1[0] = a[2] (mutually exclusive on a[1])
    // (and lb_a = sa1[1] | sa1[0] | (a[1] & ~a[2]) ... but lb_a already pre-collapsed)
