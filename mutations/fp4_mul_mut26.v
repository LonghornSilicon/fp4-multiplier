// Mutation 26 — explicit (eh_a | eh_b) shared signal.
// Used in: K=0 test (= ~(eh_a | eh_b)), Y[0] mask, lb_a / lb_b paths.
// Hypothesis: making it a named wire lets ABC see the sharing immediately
// and avoids duplicate sub-AIGs.
//
// Note: eh_a, eh_b are the "high exponent bit" — under σ=(0,1,2,3,6,7,4,5),
// eh_a = a[2], eh_b = b[2].
...
module fp4_mul (
    ...
    wire eh_a = a[2];
    wire eh_b = b[2];
    wire eh_or = eh_a | eh_b;          // KEY shared subexpr
    wire eh_and = eh_a & eh_b;
    wire lb_a = a[1] | a[2];
    wire lb_b = b[1] | b[2];
    wire el_a = a[1] ^ a[2];
    wire el_b = b[1] ^ b[2];
