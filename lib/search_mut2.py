"""Wide remap sweep with mut2-form Verilog. Two-stage."""
...
from gen_mut2 import emit_mut2_verilog
import re, shutil, subprocess, tempfile

REPO = Path(__file__).resolve().parent.parent
LEDGER = REPO / "results_mut2.tsv"
LIB = Path(__file__).resolve().parent / "contest.lib"

FAST_SCRIPT = "strash; ifraig; scorr; dc2; strash; dch -f; map -a -B 0"
DEEP_SCRIPT = (f"strash; ifraig; scorr; dc2; strash; {RESYN2}; "
...
def synth_one(perm, abc_script, timeout=30):
    ...
    verilog = emit_mut2_verilog(values)
    with tempfile.TemporaryDirectory(prefix="sm2_", dir="/tmp") as td:
        td = Path(td)
...
def worker(args):
    ...
    script = FAST_SCRIPT if sk == "fast" else DEEP_SCRIPT
...
def main():
    ...
    args = p.parse_args()
    write_header()
    print(f"=== FAST mut2-form over first {args.n} sign-sym remaps ===", flush=True)
    perms = []
    ...
    print(f"\nStage 1 done. Top 20 by FAST:", flush=True)
    for p, g in fast_ok[:20]: print(f"  {g}: {p}")
