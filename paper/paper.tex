...
\begin{abstract}
We present a \textbf{88-gate} Boolean circuit that correctly computes the product of two
...
product structure of the FP4 format, ultimately reducing gate count by $69\%$.
...
the gate count from $\sim$288 (na\"ive) to \textbf{88} gates.
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[
    ...
    \node[block, below=0.3cm of mag] (cond) {Stage 4: Cond.\ Negation\\carry chain ($r_8 = m_0$)\\22 gates};
  ...
  \end{tikzpicture}
  \caption{5-stage multiplier pipeline. Total: 88 gates.}
  \label{fig:arch}
\end{figure}
...
The standard 8-bit conditional two's-complement negation requires 23 gates.
We identify one further saving:

\textbf{Lemma}: For any 8-bit magnitude $m$, the LSB of $-m$ in two's-complement
equals $m_0$ (the LSB of $m$).

\textit{Proof}: $-m \equiv \overline{m} + 1 \pmod{2^8}$. At bit position 0:
$(\overline{m} + 1)_0 = \overline{m_0} \oplus 1 = m_0$ (since adding 1 with
carry-in 0 at bit 0 gives $\overline{m_0} + 1$; but the carry-in to the
two's-complement is 1, so the result at bit 0 is $\overline{m_0} \oplus 1 = m_0$). $\square$

Therefore $r_8 = m_0$ always, saving 1 XOR gate. The conditional negation
costs \textbf{22 gates}.
...
so all magnitude bits $m_i = 0$. The carry chain with $m_i = 0$ produces
$r_i = 0$ for all $i$ (verified by case analysis).
...
\begin{table}[h]
  ...
  \begin{tabular}{clcc}
    ...
    \textbf{v4b} & S decoder: $\text{sh}_6=u_{11}$ & \textbf{88} & 1 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[h]
  \centering
  \caption{Final circuit gate breakdown (88 total).}
  \label{tab:breakdown}
  \begin{tabular}{lcc}
    \toprule
    ...
    \midrule
    Sign & 1 & 1.1\% \\
    Non-zero detect & 5 & 5.7\% \\
    E-sum (2-bit adder) & 7 & 8.0\% \\
    K-flags (raw) & 3 & 3.4\% \\
    K-masking & 3 & 3.4\% \\
    S decoder & 13 & 14.8\% \\
    AND-terms & 18 & 20.5\% \\
    Magnitude OR assembly & 15 & 17.0\% \\
    Conditional negation & 22 & 25.0\% \\
    Sign masking & 1 & 1.1\% \\
    \midrule
    \textbf{Total} & \textbf{88} & 100\% \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item \textbf{Conditional negation} (22 gates): Minimum ripple-carry conditional
        two's-complement for 8 bits (standard result: $3n-2$ for $n$ bits,
        with the LSB pass-through saving 1 gate).
...
\begin{itemize}
  ...
  \item \textbf{Gate count}: the maximum gates activated per input pair is 88,
...
We designed an 88-gate Boolean circuit for FP4$\times$FP4 multiplication,
achieving a $69\%$ reduction from the na\"ive flat-SOP baseline of $\sim$288 gates.
...
\begin{enumerate}
  ...
  \item Two micro-optimizations: the LSB pass-through in conditional negation
        and the $\text{sh}_6 = u_{11}$ decoder simplification.
\end{enumerate}
