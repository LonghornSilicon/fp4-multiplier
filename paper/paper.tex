...
\begin{abstract}
We present a \textbf{86-gate} Boolean circuit that correctly computes the product of two
...
the gate count from $\sim$288 (na\"ive) to \textbf{86} gates.
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[
    ...
    \node[block, below=0.3cm of mag] (cond) {Stage 4: Cond.\ Negation\\prefix-OR formula ($r_8 = m_0$)\\20 gates};
  ...
  \end{tikzpicture}
  \caption{5-stage multiplier pipeline. Total: 86 gates.}
  \label{fig:arch}
\end{figure}
...
\begin{itemize}
  \item 6 OR gates for the prefix chain: $p_2 = m_0 \vee m_1$, $p_3 = p_2 \vee m_2$, \ldots
  \item 7 AND gates: $\text{sp}_i = \text{sign} \wedge p_i$ (using $p_1 = m_0$ directly)
  \item 7 XOR gates: $r_i = m_i \oplus \text{sp}_i$, plus $r_8 = m_0$
\end{itemize}
Total: $6 + 7 + 7 = \mathbf{20}$ gates, saving 2 over the carry chain.
...
\begin{table}[h]
  ...
  \begin{tabular}{clcc}
    ...
    \textbf{v4c} & Prefix-OR conditional negation & \textbf{86} & 2 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[h]
  \centering
  \caption{Final circuit gate breakdown (86 total).}
  \label{tab:breakdown}
  \begin{tabular}{lcc}
    \toprule
    ...
    S decoder & 13 & 15.1\% \\
    AND-terms & 18 & 20.9\% \\
    Magnitude OR assembly & 15 & 17.4\% \\
    Conditional negation & 20 & 23.3\% \\
    Sign masking & 1 & 1.2\% \\
    \midrule
...
\begin{itemize}
  ...
  \item \textbf{Gate count}: the maximum gates activated per input pair is 86,
...
We designed an 86-gate Boolean circuit for FP4$\times$FP4 multiplication,
...
\begin{enumerate}
  ...
  \item Three micro-optimizations: the $\text{sh}_6 = u_{11}$ decoder simplification,
        the LSB pass-through in conditional negation, and the prefix-OR formula
        that replaces the 22-gate carry chain with a 20-gate parallel structure.
\end{enumerate}
