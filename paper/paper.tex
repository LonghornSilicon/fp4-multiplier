...
(CircuitFinderSat), we prove that $N=7,8,9$ gates are insufficient (UNSAT)
and find a \textbf{11-gate} circuit at $N=11$ in our AND/OR/XOR/NOT basis
($N=10$ is under investigation). The 11-gate circuit uses 4 AND + 2 OR + 4 XOR + 1 NOT:
\begin{align*}
...
\begin{itemize}
  ...
  \item \textbf{S decoder} (11 gates): Cirbo SAT exact synthesis proves 9 gates
        insufficient (UNSAT); 11 gates achievable (SAT). This is provably optimal
        down to $N=9$; $N=10$ remains open (SAT solver times out at 360s).
...
The S-decoder is now proven optimal down to $N=10$ by exact synthesis.
...
\begin{enumerate}
  ...
  \item \textbf{SAT/ILP exact synthesis} (in progress): Cirbo CircuitFinderSat
        confirmed the S-decoder optimal at 11 gates (N=10 remains open at 360s SAT
        budget). Applying the same technique to the E-sum+decoder (4-in 7-out, 20
        gates) and magnitude OR stage may yield further reductions.
