...
The previous session's tier-1/2 recommendations are now running on a dedicated AMD EPYC 7763 (28 cores / 56 GB RAM) Ubuntu VPS.

**Currently running:**
- eSLIM massive sweep: 140 round-1 experiments (35 starting BLIFs × 2 sizes × 2 seeds), 18-way parallel. Round 2 launcher armed (108 phase-A + 32 phase-B/C experiments) — auto-fires when round 1 completes.
- Cirbo per-output-bit SAT: walking each Y[k] up from G=7 with 4-hour budget per step. Y[8]=7 newly proven (vs prior session's only-Y[0]=4 result).
- Cirbo conditional-negate sub-block: walking G upward; lower bound now ≥ 11 gates (G=10 UNSAT in 112s, G=11 attempt active).
- 5 new structurally-distinct mutations (mut30..34) added: OR-ladder above-detector, one-hot K + SOP magnitude, Sklansky parallel-prefix, Booth-2 B-recoding, Shannon-on-sy with shared above-ladder.
- 11 gate-neutral topology variants generated by inverter-pushing rewrites (workspace/eslim_runs/gen_variants.py); all verified.

**Tighter sub-block lower bounds (proven 2026-04-27):**
- Y[8] = 7 (proven SAT/UNSAT bracket)
- Conditional negate ≥ 11
- Sum of all sub-block LBs (no sharing): ≥ 47

See `docs/campaign-2026-04-27.md` for the full snapshot.
